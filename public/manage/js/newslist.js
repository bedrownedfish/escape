...
layui.config({
	...
	form.on('submit(formDemo)', function(data) {
		...
		if(data.field.mak.length != 0) if(data.field.mak.length<8){
			layer.msg("密码不能小于8位数");
			return false;
		};
		if(data.field.mak == '12345678'){
			layer.msg("密码太简单");
			return false;
		}
		var mak = $.base64.encode($.base64.encode(data.field.mak));
		$('#maks').val(mak);
	...
	$('.layui-form-checkbox').on('click', function(event) {
		...
		$.ajax({
			type:'get',
			url: checkedurl,
			data:{id:id,dtype:dtype,kid:kid},
			cache:false,
			dataType:'JSON',
			success:function(data){
				var a = !dtype?"关闭":"开启";
				data.codes?layer.msg('币种'+a+'成功'):layer.msg('币种'+a+'失败');
				layer.close(index);
			},
		    error:function(e){
		    	layer.close(index);
